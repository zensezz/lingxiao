...
package cn.zensezz.lingxiao.client.core.shutdown;
...
@Slf4j
public class LingxiaoClientShutdownHook {

    private static String hookNamePrefix = "LingxiaoClientShutdownHook";
    ...
    public static void set(final LingxiaoClientRegisterRepository result, final Properties props) {
        String name = hookNamePrefix + "-" + hookId.incrementAndGet();
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            result.close();
        }, name));
        log.info("Add hook {}", name);
       LingxiaoClientShutdownHook.props = props;
    }
